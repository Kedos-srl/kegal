...
models:
  - llm: "ollama"           # provider: ollama | anthropic | anthropic_aws | openai | bedrock | gemini
    ...
    aws_read_timeout: 60        # botocore read timeout (default 60, bedrock/anthropic_aws)
    aws_connect_timeout: 60     # botocore connect timeout (default 60, bedrock/anthropic_aws)
    aws_retries: 3              # max retry attempts (default 3; 0 = disable)
    # Bedrock batch only:
...
| `anthropic_aws` | Claude via Bedrock | `model` (ARN), `aws_region_name`, `aws_access_key`, `aws_secret_key`; optional `aws_read_timeout`, `aws_connect_timeout`, `aws_retries` |
| `bedrock` | AWS Bedrock native | `model`, `aws_region_name`, `aws_access_key`, `aws_secret_key`; optional `aws_read_timeout`, `aws_connect_timeout`, `aws_retries`; add `batch_role_arn`, `batch_s3_input_uri`, `batch_s3_output_uri` for batch mode |
